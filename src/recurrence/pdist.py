...
def torch_pdist(X: torch.Tensor, metric: str = "euclidean", p: float = 3) -> torch.Tensor:
    X = X.float()
    if metric == 'euclidean':
        ...
        Xn = X / X.norm(p=2, dim=1, keepdim=True).clamp_min(1e-12)
        sim = Xn @ Xn.T
        dist = 1.0 - sim

    elif metric == 'correlation':
        Xm = X - X.mean(dim=1, keepdim=True)
        Xn = Xm / Xm.norm(p=2, dim=1, keepdim=True).clamp_min(1e-12)
        sim = Xn @ Xn.T
        dist = 1.0 - sim
