...
class RawMTFConcatClassifier(nn.Module):
    """
    raw time series + MTF image через concat fusion.
    ...
    def forward(self, x_raw: torch.Tensor, x_mtf: torch.Tensor) -> torch.Tensor:
        h_raw = self.raw_encoder(x_raw)
        h_mtf = self.image_encoder(x_mtf)
        h_final = self.fusion(h_raw, h_mtf)
